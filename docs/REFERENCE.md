# LongHorizonAgent (LHA): full reference

> The short overview, architecture diagrams and screens are in the [README](../README.md). This page has
> every detail: the inspector, the benchmarks, configuration, flags, the CAD tools, and the demo script.
...
![A plate the agent designed, shown in the dashboard with its spec, grade and build123d script](images/cad-part-pass.png)
...
![Memory inspector tab at step 822: an ECO overwrites a fact, the old value forks to the archive, and the naive context keeps growing](images/memory-inspector-mutation.png)

![Memory inspector tab: the working state, the state mutation, memory against the naive transcript, and the build pipeline ending in the validator](images/memory-inspector.png)
...
![New design: a Design Intent box with example requests and Create Design](images/design-intent-new.png)
...
![A Star of David from intent: an outline in sterling silver, then ECO-9 switches it to gold; validated against an independent reference](images/design-intent-star.png)

![A water bottle from intent: revolved in PET, ECO-12 thins the wall to 1.5 mm, capacity 779 ml](images/design-intent-bottle.png)
...
![A design from intent: the request and its interpretation, the validated CAD, the current understanding, and the evolution through a user answer and ECO-1847](images/design-intent.png)
...
![Replay of an --intent run: the original request, the last engineering changes to that part, and its spec now](images/design-intent-replay.png)
...
![Current Design Understanding next to the working state and the state mutation at step 822](images/design-understanding.png)
...
![A wrong part: see-through, with the red reference outline showing it is 2 mm too thick](images/cad-part-fail.png)
...
![Benchmarks tab: at 3,000 messages the naive prompt climbs to 161k tokens while the stateful prompt stays at 1.3k](images/dashboard-benchmarks.png)
